...
define("ip", default="127.0.0.1", help="ip of the kernel gateway server")
define("port", default=8888, help="port of the kernel gateway server")
...
def main():
    ...
    ip = options.ip
    port = options.port
    base_url = os.getenv('BASE_GATEWAY_HTTP_URL', 'http://%s:%d' % (ip, port))
    base_ws_url = os.getenv('BASE_GATEWAY_WS_URL', 'ws://%s:%d' % (ip, port))
